...
def getdataset(pathlist,subfile,mode,train,dev):#pathlist:file_path_list,subfile:subfile_name_list,train,dev:choose the train,dev,test part of all files
    ...
    return lens,datalist[0:int(train*lens)],datalist[int(train*lens):int(dev*lens)],datalist[int(dev*lens):lens]
...
def main():
    ...
    lens,trainlist,devlist,testlist = getdataset(filepathlist,subfile,mode,0.8,0.9)
    print('Find:',lens)

    maxpiece = int(input("please input the quantity you want:").strip())
    trainlist = trainlist[0:int(maxpiece*0.8)]
    devlist = devlist[0:int(maxpiece*0.1)]
    testlist = testlist[0:int(maxpiece*0.1)]
